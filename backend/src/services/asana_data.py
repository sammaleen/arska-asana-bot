...
def format_df__(df, extra_note, max_len=None, max_note_len=None):
    ...
    for project, group in grouped_tasks:
        # Escape project name
        project_name = project if project else 'No project'
        project_name_escaped = project_name.replace("<", "&lt;").replace(">", "&gt;").replace("&", "&amp;")
        ...
        for idx, row in enumerate(sorted_group, start=1):
            ...
            # Escape and truncate notes
            notes_escaped = notes.replace("<", "&lt;").replace(">", "&gt;").replace("&", "&amp;")
            if max_note_len and len(notes_escaped) > max_note_len:
                notes_escaped = notes_escaped[:max_note_len - 3].rstrip() + " (...)"
    ...
    if extra_note:
        # Escape and truncate extra note
        extra_note_escaped = extra_note.replace("<", "&lt;").replace(">", "&gt;").replace("&", "&amp;")
        if max_note_len and len(extra_note_escaped) > max_note_len:
            extra_note_escaped = extra_note_escaped[:max_note_len - 3].rstrip() + " (...)"
        message += f"<b>✲ Note:</b>\n{extra_note_escaped}\n\n"
